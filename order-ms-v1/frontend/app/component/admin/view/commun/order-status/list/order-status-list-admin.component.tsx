...
import Edit from '../edit/order-status-edit-admin.component';
import Create from '../create/order-status-create-admin.component';
import View from '../view/order-status-view-admin.component';
...
const List = () => {
    ...
    const emptyCriteria = new OrderStatusCriteria();
    const service = new OrderStatusAdminService();
    ...
    } = useListHook<OrderStatusDto, OrderStatusCriteria>({ emptyItem, emptyCriteria,service, t})
    ...
    <div className="flex flex-column md:flex-row md:justify-content-between md:align-items-center">
        <h5 className="m-0">{t("orderStatus.header")}</h5>
    ...
    <div className="grid crud-demo">
        <div className="col-12">
            <div className="card">
                ...
                <Card title={t("search")} className="mb-5">
                        <div className="grid">
                            <div className="flex flex-column col-3">
                                <label className="mb-1"  htmlFor="1">{{'orderStatus.libelle' | translate}}</label>
                                <InputText id="1" value={criteria.libelle} onChange={(e) => setCriteria({ ...criteria, libelle: e.target.value })} />
                            </div>
                            <div className="flex flex-column col-3">
                                <label className="mb-1"  htmlFor="2">{{'orderStatus.code' | translate}}</label>
                                <InputText id="2" value={criteria.code} onChange={(e) => setCriteria({ ...criteria, code: e.target.value })} />
                            </div>
                            <div className="flex flex-column col-3">
                                <label className="mb-1"  htmlFor="3">{{'orderStatus.style' | translate}}</label>
                ...
                <DataTable ref={dt} value={items} selection={selectedItems} onSelectionChange={(e) => setSelectedItems(e.value as OrderStatusDto[])} dataKey="id" className="datatable-responsive" globalFilter={globalFilter} header={header} responsiveLayout="scroll" >
                    <Column selectionMode="multiple" headerStyle={{ width: '4rem' }}> </Column>
                    
                    <Column field="libelle" header={{'orderStatus.libelle' | translate}} sortable></Column>
                    
                    
                    <Column field="code" header={{'orderStatus.code' | translate}} sortable></Column>
                    
                    
                    <Column field="style" header={{'orderStatus.style' | translate}} sortable></Column>
                ...
                <Dialog visible={deleteItemDialog} style={{width: '450px'}} header={t("confirm")} modal footer={deleteItemDialogFooter} onHide={hideDeleteItemDialog}>
                    ...
                    {item && (<span>{t("orderStatus.deleteOrderStatusConfirmationMessage")} <b>{item.libelle}</b>?</span>)}
                    </div>
                </Dialog>
            ...
            <Dialog visible={deleteItemsDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteItemsDialogFooter} onHide={hideDeleteItemsDialog} >
                <div className="flex align-items-center justify-content-center">
                    <i className="pi pi-exclamation-triangle mr-3" style={{ fontSize: '2rem' }} />
                    {item && <span>{t("orderStatus.deleteOrderStatussConfirmationMessage")}</span>}
                </div>
            </Dialog>
