...
type OrderBoutiqueCreateAdminType = {
    visible: boolean,
    onClose: () => void,
    add: (item: OrderBoutiqueDto) => void,
    showToast: React.Ref<Toast>,
    list: OrderBoutiqueDto[],
    service: OrderBoutiqueAdminService,
    t: TFunction
}
const Create: React.FC<OrderBoutiqueCreateAdminType> = ({visible, onClose, add, showToast, list, service, t}) => {
    ...
    const emptyItem = new OrderBoutiqueDto();
        const {
            ...
            saveItem,
            formateDate
        } = useCreateHook<OrderBoutiqueDto, OrderBoutiqueCriteria>({list, emptyItem, onClose, add, showToast,service, isFormValid})
    ...
    const onBooleanInputChangeOrderLines = (e: InputSwitchChangeEvent, name: string) => {
       const val = e.value;
            ...
            <TabPanel header={{'orderBoutique.tabPan' | translate}}>
                <div className="formgrid grid">
                    ...
                    <div className="field col-6">
                        ...
                        <Calendar id="orderDate" value={item.orderDate} onChange={(e) => onInputDateChange(e, 'orderDate')} dateFormat="dd/mm/yy"  showIcon={true} />
                    </div>
                    <div className="field col-6">
                    ...
                    <div className="field col-6">
                        ...
                        <Dropdown  id="orderStatusDropdown"  value={item.orderStatus} options={orderStatuss} onChange={(e) => onDropdownChange(e, 'orderStatus')}   placeholder={{'orderBoutique.orderStatusPlaceHolder' | translate}} filter filterPlaceholder={{'orderBoutique.orderStatusPlaceHolderFilter' | translate}} optionLabel="libelle" showClear/>
                    </div>
                </div>
            </TabPanel>
            <TabPanel header={{'orderBoutique.orderLines' | translate}}>
                        <div className="grid">
                            ...
                            <div className="field col-6">
                                ...
                                <Dropdown id="productDropdown" value={orderLines.product} options={products} onChange={(e) => onDropdownChangeOrderLines(e, 'product')}    placeholder={{'orderLine.productPlaceHolder' | translate}} filter filterPlaceholder={{'orderLine.productPlaceHolderFilter' | translate}}  optionLabel="libelle" showClear />
                             </div>
                    ...
                    <DataTable value={item.orderLines} tableStyle={{minWidth: '50rem'}} dataKey="id">
                        ...
                        <Button icon="pi pi-pencil" rounded severity="success" className="mr-2" onClick={()=> editOrderLines(rowData)} /> </div>)}></Column>
                    </DataTable>
                    </div>
                    </div>
            </TabPanel>
