...
import Edit from '../edit/product-category-edit-admin.component';
import Create from '../create/product-category-create-admin.component';
import View from '../view/product-category-view-admin.component';
...
const List = () => {
    ...
    const emptyCriteria = new ProductCategoryCriteria();
    const service = new ProductCategoryAdminService();
    ...
    } = useListHook<ProductCategoryDto, ProductCategoryCriteria>({ emptyItem, emptyCriteria,service, t})
    ...
    <div className="flex flex-column md:flex-row md:justify-content-between md:align-items-center">
        <h5 className="m-0">{t("productCategory.header")}</h5>
    ...
    <div className="grid crud-demo">
        <div className="col-12">
            <div className="card">
                ...
                <Card title={t("search")} className="mb-5">
                        <div className="grid">
                            <div className="flex flex-column col-3">
                                <label className="mb-1"  htmlFor="1">{{'productCategory.libelle' | translate}}</label>
                                <InputText id="1" value={criteria.libelle} onChange={(e) => setCriteria({ ...criteria, libelle: e.target.value })} />
                            </div>
                            <div className="flex flex-column col-3">
                                <label className="mb-1"  htmlFor="2">{{'productCategory.code' | translate}}</label>
                ...
                <DataTable ref={dt} value={items} selection={selectedItems} onSelectionChange={(e) => setSelectedItems(e.value as ProductCategoryDto[])} dataKey="id" className="datatable-responsive" globalFilter={globalFilter} header={header} responsiveLayout="scroll" >
                    <Column selectionMode="multiple" headerStyle={{ width: '4rem' }}> </Column>
                    
                    <Column field="libelle" header={{'productCategory.libelle' | translate}} sortable></Column>
                    
                    
                    <Column field="code" header={{'productCategory.code' | translate}} sortable></Column>
                ...
                <Dialog visible={deleteItemDialog} style={{width: '450px'}} header={t("confirm")} modal footer={deleteItemDialogFooter} onHide={hideDeleteItemDialog}>
                    ...
                    {item && (<span>{t("productCategory.deleteProductCategoryConfirmationMessage")} <b>{item.libelle}</b>?</span>)}
                    </div>
                </Dialog>
            ...
            <Dialog visible={deleteItemsDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteItemsDialogFooter} onHide={hideDeleteItemsDialog} >
                <div className="flex align-items-center justify-content-center">
                    <i className="pi pi-exclamation-triangle mr-3" style={{ fontSize: '2rem' }} />
                    {item && <span>{t("productCategory.deleteProductCategorysConfirmationMessage")}</span>}
                </div>
            </Dialog>
